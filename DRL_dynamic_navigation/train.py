...
####### train1 ###########
# sampling with orca, normal DQN learning
####### train1 ###########
...
def run_k_episodes(k, phase, epsilon, if_update_memory=False, if_initialize_memory=False, episode=None):
    ...
    for i in range(k):
        ...
        while not done:
            ...
            if args.policy == 'sac' or args.policy == 'ddpg' or args.policy == 'sac_simple': # continuous action
                if not if_initialize_memory:
                    ...
                    if phase == 'train' and probability < epsilon:
                        action_noise = np.random.normal(0.0, 0.3, size=2)
                        action = np.clip(action + action_noise, -1.0, 1.0)
                    elif phase == 'train' and probability >= epsilon:
                        if args.policy == 'sac' or args.policy == 'sac_simple':
                            action = target_policy.get_action(lidar, position)
                        else:
                            action = target_policy.get_action(lidar, position, decay=epsilon)
                    else:
                        if args.policy == 'sac' or args.policy == 'sac_simple':
                            action = target_policy.get_action(lidar, position)
                        else:
                            action = target_policy.get_action(lidar, position, decay=0.0)
            ...
            if args.policy == 'sac' or args.policy == 'ddpg' or args.policy == 'sac_simple':
...
def main():
    ...
    while episode < train_episodes:
        ...
        # evaluate the model
        # if episode % evaluation_interval == 0:
        #    run_k_episodes(env.case_size['val'], 'val', episode=(episode + 1))
        ...
        if episode % checkpoint_interval == 0:
            if args.policy == 'sac':
                weight_file = args.output_dir + '/weight_episode_sac_{}'.format(episode)
                target_policy.save_model(weight_file)
            elif args.policy == 'sac_simple':
                weight_file = args.output_dir + '/weight_episode_sac_simple_{}'.format(episode)
                target_policy.save_model(weight_file)
            elif args.policy == 'ddpg':
                weight_file = args.output_dir + '/weight_episode_ddpg_{}'.format(episode)
                target_policy.save_model(weight_file)
            else:
                weight_file = args.output_dir + '/weight_episode_v3_{}.h5'.format(episode)
                target_policy.save_model(weight_file)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epsilon_decay', type=int, default='16000')
